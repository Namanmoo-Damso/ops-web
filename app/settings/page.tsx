...
// Convert API response to SettingsState
function apiToState(api: {
  ...
  return {
    ...
    riskDetection: {
      sensitivity: api.riskSensitivity as 1 | 2 | 3,
    },
    conversationTopics: {
      healthCheck: api.healthCheck,
      mealCheck: api.mealCheck,
      medicationCheck: api.medicationCheck,
      sleepCheck: api.sleepCheck,
      moodCheck: api.moodCheck,
    },
    scheduledCalls: {
      preferredStartTime: api.preferredStartTime,
      preferredEndTime: api.preferredEndTime,
    },
  };
}

// Convert SettingsState to API request
function stateToApi(state: SettingsState) {
  return {
    preferredStartTime: state.scheduledCalls.preferredStartTime,
    preferredEndTime: state.scheduledCalls.preferredEndTime,
    maxRetries: state.retryPolicy.maxRetries,
    retryInterval: state.retryPolicy.retryInterval,
    riskSensitivity: state.riskDetection.sensitivity,
    healthCheck: state.conversationTopics.healthCheck,
    mealCheck: state.conversationTopics.mealCheck,
    medicationCheck: state.conversationTopics.medicationCheck,
    sleepCheck: state.conversationTopics.sleepCheck,
    moodCheck: state.conversationTopics.moodCheck,
  };
}
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      if (apiSettings) {
        const stateSettings = apiToState(apiSettings);
        dispatch({ type: 'LOAD_SETTINGS', payload: stateSettings });
        setSavedSettings(stateSettings);
      }
      setIsLoading(false);
    };
    loadSettings();
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const result = await settingsApi.updateSettings(stateToApi(settings));
      ...
      if (result) {
        ...
        showToast(
          settingsApi.error || '설정 저장 중 오류가 발생했습니다.',
          'error',
        );
      }
    } catch (error) {
  ...
  const currentSensitivityInfo =
    SENSITIVITY_DESCRIPTIONS[settings.riskDetection.sensitivity];
      ...
      <div className="settings-container">
        ...
        <Card padding="lg" className="settings-section">
          ...
          <div className="settings-form-row">
            <TimePicker
              ...
              onChange={value =>
                dispatch({ type: 'SET_SCHEDULED_START_TIME', payload: value })
              }
              fullWidth
            />
            <TimePicker
              ...
              onChange={value =>
                dispatch({ type: 'SET_SCHEDULED_END_TIME', payload: value })
              }
              fullWidth
            />
          </div>
          ...
          <div className="settings-warning-banner">
            ...
            <div className="settings-warning-content">
              <div className="settings-warning-text">
                설정된 횟수만큼 재시도 후에도 연락이 닿지 않으면 담당자에게
                부재중 알림이 발송됩니다.
              </div>
            </div>
          </div>
        ...
        <Card padding="lg" className="settings-section">
          ...
          <Slider
            ...
            onChange={value =>
              dispatch({
                type: 'SET_RISK_SENSITIVITY',
                payload: value as 1 | 2 | 3,
              })
            }
            min={1}
            max={3}
              ...
              <div className="settings-sensitivity-examples">
                ...
                {currentSensitivityInfo.examples.map((example, index) => (
                  <div key={index} className="settings-sensitivity-example">
                    <span className="settings-sensitivity-example-icon">
                      💬
                    </span>
                    <span>"{example}"</span>
                  </div>
          ...
          <div className="settings-topic-list">
            <div className="settings-topic-item">
              ...
              <Switch
                ...
                onChange={() =>
                  dispatch({
                    type: 'TOGGLE_CONVERSATION_TOPIC',
                    payload: 'healthCheck',
                  })
                }
                aria-label="건강 상태 확인"
