...
const MOCK_ITEMS: BulletinItem[] = [
  {
    ...
    date: '01/11',
    title: '시스템 점검 안내',
    content: '01/15 02:00-04:00 시스템 점검이 예정되어 있습니다.',
    author: '관리자',
    attachments: [{ name: '점검일정.pdf', url: '#', size: '128KB' }],
  },
  {
    id: '2',
    date: '01/10',
    title: '신규 직원 교육',
    content: '신규 직원 교육 일정을 확인해주세요.',
    author: '김팀장',
  },
  {
    id: '3',
    date: '01/08',
    title: '알고리즘 업데이트',
    content: '위급 감지 알고리즘이 업데이트 되었습니다.',
    author: '시스템',
    attachments: [{ name: '업데이트노트.docx', url: '#', size: '45KB' }],
  },
  {
    id: '4',
    date: '01/07',
    title: '휴무일 공지',
    content: '설 연휴 휴무 안내입니다.',
    author: '관리자',
  },
  {
    id: '5',
    ...
    <>
      <Card padding="md" className="dashboard-bulletin">
        <div className="dashboard-bulletin-header">
          ...
          <Button
            ...
            className="dashboard-bulletin-add-btn"
          >
            <Plus size={16} />
            <span>새 글</span>
          </Button>
        ...
        <div className="dashboard-bulletin-content">
          {loading ? (
            <div className="dashboard-bulletin-empty">로딩 중...</div>
          ) : paginatedItems.length === 0 ? (
            <div className="dashboard-bulletin-empty">
            ...
            paginatedItems.map(item => (
              <div
                key={item.id}
                className="dashboard-bulletin-item"
              ...
              >
                ...
                <span className="dashboard-bulletin-text">
                  ...
                  {item.attachments && item.attachments.length > 0 && (
                    <Paperclip
                      size={14}
                      className="dashboard-bulletin-attachment-icon"
                    />
                  )}
                </span>
                <span className="dashboard-bulletin-author">{item.author}</span>
                <div
                  className="dashboard-bulletin-actions dashboard-bulletin-actions-visible"
                  onClick={e => e.stopPropagation()}
                >
                  <button
                    className="dashboard-bulletin-action"
                    onClick={() => handleOpenEdit(item)}
                  >
                    <Edit2 size={16} />
                  </button>
                  <button
                    className="dashboard-bulletin-action dashboard-bulletin-action-danger"
      ...
      {detailItem && (
        <div className="bulletin-overlay" onClick={handleCloseDetail}>
          <div
            className="bulletin-dialog bulletin-dialog-detail"
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className="bulletin-dialog-body">
              ...
              {detailItem.attachments && detailItem.attachments.length > 0 && (
                <div className="bulletin-detail-attachments">
                  <h4 className="bulletin-detail-attachments-title">
                    첨부파일
                  </h4>
                  {detailItem.attachments.map((file, idx) => (
                    <a
                      ...
                      href={file.url}
                      download
                      className="bulletin-detail-attachment"
                    >
                      <Paperclip size={16} />
                      <span>{file.name}</span>
                      {file.size && (
                        <span className="bulletin-detail-attachment-size">
                          {file.size}
                        </span>
                      )}
                      <Download
                        size={16}
                        className="bulletin-detail-download-icon"
                      />
                    </a>
                  ))}
                </div>
      ...
      {modalOpen && (
        <div className="bulletin-overlay" onClick={handleClose}>
          <div className="bulletin-dialog" onClick={e => e.stopPropagation()}>
            ...
            <div className="bulletin-dialog-footer">
              <button className="bulletin-dialog-cancel" onClick={handleClose}>
                취소
              </button>
