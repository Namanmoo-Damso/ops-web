...
통계 페이지(`/app/stats`)와 직원관리 페이지(`/app/staff`)는 현재 대부분 데이터를 Mock 데이터로 표시합니다. 이 문서는 기존 스키마로 연동 가능한 데이터와 추가 스키마가 필요한 데이터를 정리합니다.
...
| 직원 관리 | `/app/staff` | 직원 목록, 업무 부하, 대상자 배정 |
